...
import { getNotifications, markNotificationRead } from '@/services/authorityService';
...
export default function AuthorityNotificationsPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-navy-900">Authority Notifications</h1>
      ...
      {notifications.length === 0 ? (
        ...
        <ul className="space-y-3">
          {notifications.map((n) => (
            <li
              key={n._id}
              className={`rounded-xl border border-navy-100 bg-white p-4 flex items-start justify-between gap-4 ${
                !n.isRead ? 'border-l-4 border-l-grace-cyan' : ''
              }`}
            >
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {!n.isRead && (
                    <span className="rounded-full bg-grace-cyan px-2 py-0.5 text-xs text-white">New</span>
                  )}
                </div>
              ...
              {!n.isRead && (
                <button
                  ...
                  className="shrink-0 text-xs font-medium text-grace-cyan hover:underline"
                >
                  Mark read
                </button>
              )}
